...
def bake_legend(zone_results, params, status_cb):
    ...
    origin = rg.Point3d(minx - 8.0 * scale, miny - 18.0 * scale, 0)

    lines = [
        "LEGEND",
        "Gold — REGISTRATION — the one compulsory gate",
        "Green — EDUCATION — judgment trained, then kept alive",
        "Tan — EMPLOYMENT — hours in practice; project is a loop",
        "Blue — LEGISLATION — Act and NCC can be rewritten",
        "Purple — BODIES — Institute advances; Board accounts",
        "Rose — ETHICS — duty past the fee-payer",
        "Navy — PROCUREMENT — contract can strip design control",
        "Teal — RESEARCH — codes change what may be built",
        "Terracotta — POWER — tight title, loose code and contract",
        "Arcs: enables / limits / depends upon / influences / contrasts",
    ]
    step = pt_to_mm(params.legend_pt, params) * 2.6
    add_label(origin, lines[0], params.heading_pt, (210, 210, 210), layer, "LEGEND", params, justify=1)
    for i, line in enumerate(lines[1:]):
        pt = rg.Point3d(origin.X, origin.Y - (i + 1) * max(step, 1.4 * scale), 0)
        add_label(pt, line, params.legend_pt, (180, 180, 180), layer, "LEGEND", params, justify=1)
    status_cb("Legend ({} pt) at lower-left.".format(params.legend_pt))
...
def generate_diagram(params, status_cb):
    ...
    rs.ZoomExtents()
    status_cb(
        "V7 clean links + Futura. Hide pipes before vector export.\n"
...
class StigmergyForm(eforms.Form):
    ...
    def __init__(self):
        ...
        self.Title = "A1 Stigmergy  V7  v1.0  (clean links + Futura text)"
        self.Resizable = True
    ...
    def _build_ui(self):
        ...
        self._draw_legend = w.check("Legend lower-left", True)
        ...
        self.Content = outer
        self.status_cb(
            "Clean default = pic 2 (highways + relations; nerve/junctions off).\n"
            "Tick nerve sub-links for pic 1 density. Text = Futura at pt sizes."
        )
