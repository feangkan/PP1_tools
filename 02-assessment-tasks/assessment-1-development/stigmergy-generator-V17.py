"""
Assessment 1 stigmergy generator  V17 / v2.0
...
Futura catalog text defaults black (pic 2). Relation / loop text has its own
mm + colour. Every label gets a white mask stroke and is brought to the front
so disks / diamonds cannot hide the words. Relation diamonds are 70% smaller.
Text sizes are millimetres. After Generate, Update text restyles without redraw.
Annotation scale 1:1.

LAYOUT  v1.4 -- mycelial / nerve mesh (fast + animated)
------------------------------------------------------------
Catalog text is middle-centered ON each node (not offset beside it).
Group linkages are a few smooth highway curves (not a 26-strand scribble),
...
class Widgets(object):
    ...
    def combo(self, items, index=0, width=240):
        cb = eforms.ComboBox()
        cb.DataStore = list(items)
        cb.SelectedIndex = index
        cb.Width = width
        try:
            cb.BackgroundColor = TH["bg_ctrl"]
            cb.TextColor = TH["fg_bright"]
        except Exception:
            pass
        return cb

    def color_picker(self, rgb=(0, 0, 0)):
        r, g, b = [int(max(0, min(255, v))) for v in rgb]
        try:
            cp = eforms.ColorPicker()
            try:
                cp.Value = edrawing.Color.FromArgb(255, r, g, b)
            except Exception:
                cp.Value = edrawing.Color(r / 255.0, g / 255.0, b / 255.0)
            cp.Width = 78
            return cp
        except Exception:
            return self.textbox("{:02X}{:02X}{:02X}".format(r, g, b), width=78)

    def row2(self, label, ctrl_a, ctrl_b, lbl_w=160):
        tl = eforms.TableLayout()
        tl.Spacing = edrawing.Size(5, 0)
        spacer = eforms.TableCell(eforms.Label())
        spacer.ScaleWidth = True
        tl.Rows.Add(eforms.TableRow(
            eforms.TableCell(self.lbl(label, width=lbl_w)),
            eforms.TableCell(ctrl_a),
            eforms.TableCell(ctrl_b),
            spacer,
        ))
        return tl


def rgb_from_picker(ctrl):
    """Read an Eto ColorPicker (or hex textbox fallback) as (R, G, B) 0–255."""
    if ctrl is None:
        return (0, 0, 0)
    if isinstance(ctrl, eforms.TextBox):
        raw = (ctrl.Text or "000000").strip().lstrip("#")
        if len(raw) == 6:
            try:
                ...
                return (0, 0, 0)
        return (0, 0, 0)
    c = getattr(ctrl, "Value", None)
    if c is None:
        return (0, 0, 0)
    rb = getattr(c, "Rb", None)
    if rb is not None:
        return (int(c.Rb), int(c.Gb), int(c.Bb))
    try:
        return (
            int(round(float(c.R) * 255.0)),
            int(round(float(c.G) * 255.0)),
            int(round(float(c.B) * 255.0)),
        )
    except Exception:
        return (0, 0, 0)
...
class GrowthParams(object):
    def __init__(self):
        ...
        # Pic 3: relation diamonds 70% smaller than V16's 3.0 → 0.9
        self.rel_dot_radius = 0.9
        self.junction_radius = 2.2
        self.highway_line = 0.85
        self.highway_strands = 3
        self.trunk_line = 0.90
        self.branch_line = 0.26
        self.hair_line = 0.08
        self.rel_line = 0.50
        self.mesh_link_dist = 28.0
        self.mesh_sample = 14
        self.capillary_hairs = 8
        self.bridges_per_pair = 7
        self.bridge_range = 150.0
        self.draw_title_ellipse = True
        self.draw_highways = True
        self.draw_highway_filaments = False
        self.draw_mesh_links = False
        ...
        # Pic 2 defaults (mm) + black chips; relation/loop has its own size
        self.title_pt = 3.0
        self.heading_pt = 3.0
        self.subtopic_pt = 1.0
        self.branch_pt = 0.1
        self.legend_pt = 1.5
        self.relation_pt = 1.0
        self.text_scale = 1.0
        self.text_stroke = True
        self.text_stroke_colour = (255, 255, 255)
        self.text_stroke_offset = 0.18
        self.use_group_colour = False
        self.title_colour = (0, 0, 0)
        self.subtitle_colour = (0, 0, 0)
        self.heading_colour = (0, 0, 0)
        self.subtopic_colour = (0, 0, 0)
        self.branch_colour = (0, 0, 0)
        self.legend_colour = (0, 0, 0)
        self.relation_colour = (0, 0, 0)
        self.fast_mode = True
...
def text_height_mm(size, params):
    """Rhino text height in millimetres. GUI 2.0 = 2.0 mm in Properties."""
    scale = max(float(getattr(params, "text_scale", 1.0)), 0.01)
    return max(float(size) * scale, 0.05)


def pt_to_mm(pt_size, params):
    return text_height_mm(pt_size, params)


# Rhinoscript AddText: 1–9 keypad, 5 = middle-center (text sits ON the node).
JUST_NODE = 5
JUST_LEGEND = 1
JUST_MIDDLE_CENTER_FLAGS = 2 | (1 << 17)  # Rhino.Geometry.TextJustification.MiddleCenter
TEXT_DISPLAY_ORDER = 200


def _drawing_color(rgb, alpha=255):
    r, g, b = rgb
    try:
        return sdrawing.Color.FromArgb(int(alpha), int(r), int(g), int(b))
    except Exception:
        return sdrawing.Color.White


def bring_object_to_front(oid, order=TEXT_DISPLAY_ORDER):
    """Raise draw order so disks / diamonds cannot cover text."""
    if not oid:
        return
    try:
        if hasattr(rs, "BringObjectToFront"):
            rs.BringObjectToFront(oid)
    except Exception:
        pass
    guid = None
    try:
    ...
    try:
        ...
        attr.DisplayOrder = int(order)
        sc.doc.Objects.ModifyAttributes(guid, attr, True)
    except Exception:
        pass


def apply_white_stroke(tid, params=None):
    """White annotation mask around letters (readable on coloured disks)."""
    if not tid:
        return
    if params is not None and not getattr(params, "text_stroke", True):
        bring_object_to_front(tid)
        return
    stroke_rgb = (255, 255, 255)
    offset = 0.18
    if params is not None:
        stroke_rgb = getattr(params, "text_stroke_colour", stroke_rgb)
        offset = float(getattr(params, "text_stroke_offset", offset))
    guid = None
    try:
        guid = rs.coerceguid(tid)
    except Exception:
        guid = None
    if not guid:
        bring_object_to_front(tid)
    ...
    try:
        ...
        changed = False
        for flag_name in ("MaskEnabled", "DrawTextMask"):
            if hasattr(geom, flag_name):
                try:
                    setattr(geom, flag_name, True)
                    changed = True
                except Exception:
                    pass
        if hasattr(geom, "MaskColor"):
            try:
                geom.MaskColor = _drawing_color(stroke_rgb)
                changed = True
            except Exception:
                pass
        if hasattr(geom, "MaskOffset"):
            try:
                geom.MaskOffset = max(offset, 0.05)
                changed = True
            except Exception:
                pass
        # 1 = rectangle frame under text when available
        if hasattr(geom, "MaskFrame"):
            try:
                geom.MaskFrame = 1
                changed = True
            except Exception:
                pass
        if hasattr(geom, "MaskFrameType"):
            try:
                geom.MaskFrameType = 1
                changed = True
            except Exception:
                pass
        if changed:
            sc.doc.Objects.Replace(guid, geom)
    except Exception:
        pass
    bring_object_to_front(tid)


def raise_text_layers():
    """Keep text / legend layers after geometry so bake order stays readable."""
    for name in ("Diagram::Text", "Diagram::Legend"):
        try:
            if rs.IsLayer(name):
...
def add_label(pt, text, pt_size, colour, layer, name, params, justify=JUST_NODE, kind=None):
    """Place Futura on a point. White stroke + front draw order (pic 1)."""
    if not text or not params.draw_text:
        return None
    height = text_height_mm(pt_size, params)
    font = resolve_font(params.font_name)
    plane = rg.Plane(pt, rg.Vector3d.ZAxis)
    tid = None
    for j in (justify, JUST_MIDDLE_CENTER_FLAGS, JUST_NODE, 2):
        try:
            tid = rs.AddText(str(text), plane, height, font, 0, j)
        except Exception:
            tid = None
        if tid:
            break
    ...
    try:
        ...
        pass
    for j in (justify, JUST_MIDDLE_CENTER_FLAGS, JUST_NODE):
        try:
            rs.TextObjectJustification(tid, j)
            break
        except Exception:
            continue
    if kind:
        try:
            rs.SetUserText(tid, "text_kind", kind)
        except Exception:
            pass
    apply_white_stroke(tid, params)
    return tid
...
def bake_highway_filaments(zone_results, params):
    """A few short side hairs on each highway — off by default (pic 2 mess)."""
    ...
    for a_key, b_key, weight in HIGHWAYS:
        ...
        samples = sample_curve_points(p1, p2, 5, bulge=0.14, scale=scale)
        ...
        # Two short hairs near mid-span only — not a random spray.
        mids = samples[1:-1]
        if not mids:
            continue
        for sp in mids[len(mids) // 2:len(mids) // 2 + 1]:
            dx = p2.X - p1.X
            dy = p2.Y - p1.Y
            ...
            for sign in (-1.0, 1.0):
                tip = push_outside_title(
                    rg.Point3d(sp.X + px * side * sign, sp.Y + py * side * sign, 0),
                    params,
                )
                draw_segment(sp, tip, params.hair_line * scale, blend, layer, "filament", "hair")
                count += 1
    return count
...
def bake_highways(zone_results, params):
    """Clean hub-to-hub curves (pic 1). A few smooth strands, no scribble bundle."""
    if not params.draw_highways:
        return 0
    layer = ensure_layer("Diagram::Highways", (100, 100, 100))
    rs.CurrentLayer(layer)
    scale = params.canvas_scale
    n_base = max(1, int(getattr(params, "highway_strands", 3)))
    count = 0
    ...
    for a_key, b_key, weight in HIGHWAYS:
        ...
        n = max(1, int(round(n_base * (0.65 + 0.45 * weight))))
        spread = params.highway_line * 1.25 * scale
        bulge = 0.12 + 0.06 * weight
        core = sample_curve_points(p1, p2, 8, bulge=bulge, scale=scale)
        for s in range(n):
            u = s / float(max(n - 1, 1)) if n > 1 else 0.5
            lateral = (u - 0.5) * 2.0 * spread
            walk = offset_walk(list(core), lateral)
            dist_from_core = abs(u - 0.5)
            if n == 1 or dist_from_core < 0.20:
                pw = params.branch_line * 1.55 * scale
                col = blend
            else:
                pw = params.hair_line * scale * 1.4
...
def bake_loops(zone_results, params, status_cb):
    ...
    for group, a_id, b_id, label in INTRA_LOOPS:
        ...
        set_dashed(crv)
        mid = pts[len(pts) // 2]
        text_layer = ensure_layer("Diagram::Text", (220, 220, 220))
        add_label(
            rg.Point3d(mid.X, mid.Y + 1.6 * scale, 0),
            label,
            getattr(params, "relation_pt", params.branch_pt),
            getattr(params, "relation_colour", (0, 0, 0)),
            text_layer, "{}-{}".format(a_id, b_id), params, kind="loop",
        )
        n += 1
        anim_tick()
    if n:
...
def bake_labels(zone_results, params, status_cb):
    """Catalog type sits on the node centres — not offset beside the dots."""
    if not params.draw_text:
        return
    layer = ensure_layer("Diagram::Text", (220, 220, 220))
    rs.CurrentLayer(layer)
    font = resolve_font(params.font_name)
    status_cb("Font: {}  (labels centred on nodes).".format(font))

    title_col = getattr(params, "title_colour", (0, 0, 0))
    sub_col = getattr(params, "subtitle_colour", (0, 0, 0))
    rel_col = getattr(params, "relation_colour", (0, 0, 0))
    use_group = bool(getattr(params, "use_group_colour", True))

    title_pt = title_center(params)
    add_label(
        title_pt,
        "DISCIPLINARY MATRIX", params.title_pt, title_col, layer, "TITLE", params,
        kind="title",
    )
    sub_dy = text_height_mm(params.title_pt, params) * 1.15
    add_label(
        rg.Point3d(title_pt.X, title_pt.Y - sub_dy, 0),
        "Trusted with the title — by whom, for what?",
        params.subtopic_pt, sub_col, layer, "TITLE_SUB", params,
        kind="subtitle",
    )
    ...
    for cat in CATEGORIES:
        ...
        if not zd:
            continue
        group_c = cat["colour"]
        head_c = group_c if use_group else getattr(params, "heading_colour", (0, 0, 0))
        tip_c = group_c if use_group else getattr(params, "subtopic_colour", (0, 0, 0))
        id_c = group_c if use_group else getattr(params, "branch_colour", (0, 0, 0))
        root = zd["root"]
        add_label(root, cat["key"], params.heading_pt, head_c, layer, cat["key"], params, kind="heading")
        zoom = cat.get("zoom", "")
        if zoom:
            z_dy = text_height_mm(params.heading_pt, params) * 1.2
            add_label(
                rg.Point3d(root.X, root.Y - z_dy, 0),
                zoom, params.subtopic_pt, tip_c, layer, cat["key"] + "_ZOOM", params,
                kind="zoom",
            )
        for mid, short, tip in zd.get("tips", []):
            add_label(tip, short, params.subtopic_pt, tip_c, layer, mid + "_SUB", params, kind="subtopic")
            id_dy = text_height_mm(params.subtopic_pt, params) * 0.85
            add_label(
                rg.Point3d(tip.X, tip.Y + id_dy, 0),
                mid, params.branch_pt, id_c, layer, mid, params, kind="branch",
            )
    ...
    for mid, rid, verb in zone_results.get("_rels", []):
        add_label(
            ...
            "{}  {}".format(rid, verb),
            getattr(params, "relation_pt", params.branch_pt),
            rel_col, layer, rid, params, kind="relation",
        )

    raise_text_layers()
    status_cb(
        "Text height in the model (mm): title {:.2f}, heading {:.2f}, sub {:.2f}, "
        "branch {:.2f}, relation/loop {:.2f}, legend {:.2f}. White stroke on; text in front.".format(
            text_height_mm(params.title_pt, params),
            text_height_mm(params.heading_pt, params),
            text_height_mm(params.subtopic_pt, params),
            text_height_mm(params.branch_pt, params),
            text_height_mm(getattr(params, "relation_pt", params.branch_pt), params),
            text_height_mm(params.legend_pt, params),
        )
    )
...
def bake_legend(zone_results, params, status_cb):
    ...
    title_h = text_height_mm(params.heading_pt, params)
    row_h = max(text_height_mm(params.legend_pt, params) * 2.8, 1.8 * scale)
    swatch = max(text_height_mm(params.legend_pt, params) * 0.95, 0.95 * scale)
    origin = rg.Point3d(minx - 12.0 * scale, miny - 18.0 * scale, 0)
    leg_col = getattr(params, "legend_colour", (0, 0, 0))

    add_label(
        rg.Point3d(origin.X, origin.Y, 0),
        "LEGEND", params.heading_pt, leg_col, layer, "LEGEND", params, justify=1,
        kind="legend_heading",
    )
    ...
    for i, key in enumerate(order):
        ...
        add_label(
            ...
            label, params.legend_pt, leg_col, layer, "LEGEND", params, justify=1,
            kind="legend_row",
        )
    ...
    add_label(
        ...
        params.legend_pt, leg_col, layer, "LEGEND", params, justify=1,
        kind="legend_row",
    )
    status_cb("Legend: colour circles (no Gold/Green words), heading {:.1f} mm / rows {:.1f} mm.".format(
        text_height_mm(params.heading_pt, params),
        text_height_mm(params.legend_pt, params),
    ))
...
def is_text_object(oid):
    if not oid:
        return False
    for fn in ("IsText", "IsTextObject"):
        try:
            check = getattr(rs, fn, None)
            if check and check(oid):
                return True
        except Exception:
            continue
    return False
...
def _object_layer(oid):
    try:
        return rs.ObjectLayer(oid) or ""
    except Exception:
        return ""


def _text_contents(oid):
    try:
        return (rs.TextObjectText(oid) or "").strip()
    except Exception:
        return ""


def classify_text(oid):
    kind = ""
    try:
        kind = rs.GetUserText(oid, "text_kind") or ""
    except Exception:
        kind = ""
    if kind:
        return kind
    ...
    if name == "TITLE_SUB":
        return "subtitle"
    if "Legend" in layer or name == "LEGEND":
        if _text_contents(oid) == "LEGEND":
            return "legend_heading"
        return "legend_row"
    ...
    if "-" in name:
        return "loop"
    if name and name[0] in ID_PREFIX_GROUP:
        return "branch"
    return "unknown"


def group_colour_for_name(name):
    colours = {c["key"]: c["colour"] for c in CATEGORIES}
    if name in colours:
        return colours[name]
    if name.endswith("_ZOOM"):
        return colours.get(name[:-5])
    if name.endswith("_SUB"):
        stem = name[:-4]
        if stem and stem[0] in ID_PREFIX_GROUP:
            return colours.get(ID_PREFIX_GROUP[stem[0]])
    if name and name[0] in ID_PREFIX_GROUP:
        return colours.get(ID_PREFIX_GROUP[name[0]])
    return None


def style_for_kind(kind, oid, params):
    name = _object_name(oid)
    use_group = bool(getattr(params, "use_group_colour", True))
    group_c = group_colour_for_name(name) if use_group else None
    sizes = {
        "title": params.title_pt,
        "subtitle": params.subtopic_pt,
        "heading": params.heading_pt,
        "zoom": params.subtopic_pt,
        "subtopic": params.subtopic_pt,
        "branch": params.branch_pt,
        "relation": getattr(params, "relation_pt", params.branch_pt),
        "loop": getattr(params, "relation_pt", params.branch_pt),
        "legend_heading": params.heading_pt,
        "legend_row": params.legend_pt,
        "unknown": params.subtopic_pt,
    }
    colours = {
        "title": getattr(params, "title_colour", (0, 0, 0)),
        "subtitle": getattr(params, "subtitle_colour", (0, 0, 0)),
        "heading": group_c or getattr(params, "heading_colour", (0, 0, 0)),
        "zoom": group_c or getattr(params, "subtopic_colour", (0, 0, 0)),
        "subtopic": group_c or getattr(params, "subtopic_colour", (0, 0, 0)),
        "branch": group_c or getattr(params, "branch_colour", (0, 0, 0)),
        "relation": getattr(params, "relation_colour", (0, 0, 0)),
        "loop": getattr(params, "relation_colour", (0, 0, 0)),
        "legend_heading": getattr(params, "legend_colour", (0, 0, 0)),
        "legend_row": getattr(params, "legend_colour", (0, 0, 0)),
        "unknown": getattr(params, "subtopic_colour", (0, 0, 0)),
    }
    return sizes.get(kind, params.subtopic_pt), colours.get(kind, (0, 0, 0))


def apply_text_style(tid, height, colour, font, params=None):
    try:
        rs.ObjectColorSource(tid, 1)
    except Exception:
        pass
    try:
        rs.ObjectColor(tid, colour)
    except Exception:
        pass
    try:
        rs.TextObjectHeight(tid, height)
...
def collect_diagram_text(selected_only=False):
    ids = []
    if selected_only:
        try:
            ...
            sel = []
        for oid in sel:
            if is_text_object(oid):
                ids.append(oid)
        return ids
    layers = []
    try:
        layers = [n for n in (rs.LayerNames() or []) if n.startswith("Diagram::")]
    except Exception:
        layers = ["Diagram::Text", "Diagram::Legend", "Diagram::Loops"]
    seen = set()
    for layer in layers:
        try:
            ...
            objs = []
        for oid in objs:
            if oid in seen or not is_text_object(oid):
                continue
            seen.add(oid)
            ids.append(oid)
    return ids
...
def update_baked_text(params, status_cb):
    """Restyle existing catalog text from the GUI. Does not regenerate geometry."""
    selected = collect_diagram_text(selected_only=True)
    ids = selected if selected else collect_diagram_text(selected_only=False)
    if not ids:
        status_cb("No baked text found. Generate first, then Update text.")
        return 0
    font = resolve_font(params.font_name)
    counts = {}
    for oid in ids:
        kind = classify_text(oid)
        size, colour = style_for_kind(kind, oid, params)
        apply_text_style(oid, text_height_mm(size, params), colour, font, params)
    ...
    try:
        ...
        pass
    scope = "selected text" if selected else "all diagram text"
    bits = ", ".join("{} {}".format(n, k) for k, n in sorted(counts.items()))
    status_cb("Updated {} ({}). White stroke + front. Geometry unchanged.".format(
        scope, bits or "0 objects"))
    return len(ids)
...
def generate_diagram(params, status_cb):
    ...
    try:
        ...
        raise_text_layers()
        for tid in collect_diagram_text(selected_only=False):
            bring_object_to_front(tid)
        ...
        status_cb("Done in {:.1f}s (V17 text front + white stroke{}).".format(
            elapsed,
    ...
    rs.ZoomExtents()
    status_cb(
        "V17: text in front with white stroke; relation/loop 1 mm; diamonds −70%.\n"
        "Defaults match pic 2 (black chips). Update text restyles without regenerating."
    )
...
class StigmergyForm(eforms.Form):
    ...
    def __init__(self):
        ...
        self.Title = "A1 Stigmergy  V17  v2.0  (text front + stroke)"
        self.Resizable = True
        self.MinimumSize = edrawing.Size(420, 480)
        self.ClientSize = edrawing.Size(480, 640)
    ...
    def _build_ui(self):
        ...
        self._scale = w.num(self.params.canvas_scale, 0.25, 5.0, dec=2, inc=0.05)
        ...
        self._draw_hw = w.check("Highway links (clean curves)", True)
        settings.AddRow(self._draw_hw)
        self._draw_fil = w.check("Highway side filaments", False)
        settings.AddRow(self._draw_fil)
        self._draw_mesh = w.check("Cross-link mesh (voids)", False)
        settings.AddRow(self._draw_mesh)
        self._draw_bridges = w.check("Nerve sub-links", False)
        settings.AddRow(self._draw_bridges)
        self._draw_cap = w.check("Reaching capillary hairs", True)
        settings.AddRow(self._draw_cap)
        self._draw_junc = w.check("Thickened junction nodes", False)
        ...
        self._mesh_sample = w.num(self.params.mesh_sample, 4, 120, dec=0, inc=2)
        ...
        self._strands = w.num(self.params.highway_strands, 1, 24, dec=0, inc=1)
        settings.AddRow(w.row("Strands / highway", self._strands))
        self._highway = w.num(self.params.highway_line, 0.2, 4.0, dec=2, inc=0.05)
        settings.AddRow(w.row("Highway strand spread", self._highway))
        ...
        self._rel_r = w.num(self.params.rel_dot_radius, 0.2, 10, dec=1, inc=0.1)
        ...
        settings.AddRow(w.section("Text -- Futura, sizes in mm (pic 2 defaults)"))
        ...
        self._use_group_col = w.check(
            "Hub / tip text uses group colour (off = black chips)",
            bool(self.params.use_group_colour),
        )
        settings.AddRow(self._use_group_col)
        self._text_stroke = w.check("White stroke behind text (front layer)", True)
        settings.AddRow(self._text_stroke)
        self._title_pt = w.num(self.params.title_pt, 0.1, 80, dec=1, inc=0.1)
        self._title_col = w.color_picker(self.params.title_colour)
        settings.AddRow(w.row2("Title mm", self._title_pt, self._title_col))
        self._heading_pt = w.num(self.params.heading_pt, 0.1, 80, dec=1, inc=0.1)
        self._heading_col = w.color_picker(self.params.heading_colour)
        settings.AddRow(w.row2("Heading / title mm", self._heading_pt, self._heading_col))
        self._sub_pt = w.num(self.params.subtopic_pt, 0.1, 80, dec=1, inc=0.1)
        self._sub_col = w.color_picker(self.params.subtopic_colour)
        settings.AddRow(w.row2("Subtopic mm", self._sub_pt, self._sub_col))
        self._br_pt = w.num(self.params.branch_pt, 0.1, 80, dec=1, inc=0.1)
        self._br_col = w.color_picker(self.params.branch_colour)
        settings.AddRow(w.row2("Branch / ID mm", self._br_pt, self._br_col))
        self._leg_pt = w.num(self.params.legend_pt, 0.1, 80, dec=1, inc=0.1)
        self._leg_col = w.color_picker(self.params.legend_colour)
        settings.AddRow(w.row2("Legend mm", self._leg_pt, self._leg_col))
        self._subttl_col = w.color_picker(self.params.subtitle_colour)
        settings.AddRow(w.row2("Subtitle colour", w.lbl(""), self._subttl_col))
        self._rel_pt = w.num(self.params.relation_pt, 0.1, 80, dec=1, inc=0.1)
        self._rel_col = w.color_picker(self.params.relation_colour)
        settings.AddRow(w.row2("Relation / loop mm", self._rel_pt, self._rel_col))
        self._text_scale = w.num(self.params.text_scale, 0.1, 20, dec=1, inc=0.1)
        ...
        self._btn_update = w.button("Update text", self._on_update_text, width=110, primary=True)
        ...
        outer.AddRow(w.button_row(self._btn_generate, self._btn_update, self._btn_clear))
        outer.AddRow(w.button_row(self._btn_vector, self._btn_pipes))
        outer.AddRow(self._log)
        self.Content = outer
        self.status_cb(
            "V17: white stroke + text in front. Relation/loop 1 mm. Diamonds −70%. Pic 2 black defaults."
        )
    ...
    def _read_params(self):
        ...
        p.relation_pt = float(self._rel_pt.Value)
        p.text_scale = float(self._text_scale.Value)
        p.text_stroke = bool(self._text_stroke.Checked)
        p.use_group_colour = bool(self._use_group_col.Checked)
        p.title_colour = rgb_from_picker(self._title_col)
        p.heading_colour = rgb_from_picker(self._heading_col)
        p.subtopic_colour = rgb_from_picker(self._sub_col)
        p.branch_colour = rgb_from_picker(self._br_col)
        p.legend_colour = rgb_from_picker(self._leg_col)
        p.subtitle_colour = rgb_from_picker(self._subttl_col)
        p.relation_colour = rgb_from_picker(self._rel_col)
        return p
    ...
    def _on_update_text(self, sender, e):
        self._btn_update.Enabled = False
        try:
            update_baked_text(self._read_params(), self.status_cb)
        except Exception:
            self.status_cb("ERROR:\n" + traceback.format_exc())
        finally:
            self._btn_update.Enabled = True
