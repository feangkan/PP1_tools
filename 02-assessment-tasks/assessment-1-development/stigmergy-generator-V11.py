"""
Assessment 1 stigmergy generator  V11 / v1.4
...
def bake_labels(zone_results, params, status_cb):
    ...
    sub_dy = pt_to_mm(params.title_pt, params) * 1.15
    add_label(
    ...
    for cat in CATEGORIES:
        ...
        colour = cat["colour"]
        root = zd["root"]
        add_label(root, cat["key"], params.heading_pt, colour, layer, cat["key"], params)
        zoom = cat.get("zoom", "")
        if zoom:
            z_dy = pt_to_mm(params.heading_pt, params) * 1.2
            add_label(
                rg.Point3d(root.X, root.Y - z_dy, 0),
                zoom, params.subtopic_pt, colour, layer, cat["key"] + "_ZOOM", params,
            )
        for mid, short, tip in zd.get("tips", []):
            add_label(tip, short, params.subtopic_pt, colour, layer, mid + "_SUB", params)
            id_dy = pt_to_mm(params.subtopic_pt, params) * 0.85
            add_label(
                rg.Point3d(tip.X, tip.Y + id_dy, 0),
                mid, params.branch_pt, colour, layer, mid, params,
            )
...
def generate_diagram(params, status_cb):
    ...
    rs.ZoomExtents()
    status_cb(
        "V11: catalog text sits on node centres (middle-center), not beside the dots.\n"
...
class StigmergyForm(eforms.Form):
    ...
    def __init__(self):
        ...
        self.Title = "A1 Stigmergy  V11  v1.4  (text on node centres)"
        self.Resizable = True
    ...
    def _build_ui(self):
        ...
        self.Content = outer
        self.status_cb(
            "V11: text sits on the centre of each node, not beside it.\n"
            "Linkages stay dense stigmergy lines. Legend = colour circles."
        )
