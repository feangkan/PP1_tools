"""
Assessment 1 stigmergy generator  V9 / v1.2
...
LAYOUT  v1.2 -- mycelial / nerve mesh (fast + animated)
------------------------------------------------------------
Gold REGISTRATION trunk is Victoria's linear gated pathway (Trust/Risk,
START, Stages 1-5, fail/re-queue, ejection). Other colour groups keep
their own graphic language (cycle, loop, instrument, fork, split,
contest, overlap) -- they are not forced into stages they do not have.
...
def generate_diagram(params, status_cb):
    ...
    rs.ZoomExtents()
    status_cb(
        "V9: each group keeps its own graphic language. Hide pipes before vector export.\n"
...
class StigmergyForm(eforms.Form):
    ...
    def __init__(self):
        ...
        self.Title = "A1 Stigmergy  V9  v1.2  (own graphic language per group)"
        self.Resizable = True
    ...
    def _build_ui(self):
        ...
        self.Content = outer
        self.status_cb(
            "V9: gold path = Victoria flowchart. Other groups keep their own graphic language.\n"
