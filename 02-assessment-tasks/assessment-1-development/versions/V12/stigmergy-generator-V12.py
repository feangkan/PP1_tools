"""
Assessment 1 stigmergy generator  V12 / v1.5
...
Size unit defaults to millimetres (what Rhino Properties shows).
Print-pt mode still exists (1 pt = 0.353 mm) — that is why older bakes
looked like 0.5–1 mm when the GUI said 2.0. Annotation scale 1:1.
...
def text_height_mm(size, params):
    """Rhino text height in millimetres.

    GUI numbers are *not* clamped to 0.5–1. The old print-pt unit multiplies
    by 0.353, so Title 2.0 pt became 0.71 mm — that is the 0.5–1 look.
    Default unit is millimetres: 2.0 means 2.0 mm in Properties.
    """
    scale = max(float(getattr(params, "text_scale", 1.0)), 0.01)
    unit = getattr(params, "size_unit", "mm")
    if unit == "pt":
        h = float(size) * PT_MM * scale
    else:
        h = float(size) * scale
    return max(h, 0.05)
...
def add_label(pt, text, pt_size, colour, layer, name, params, justify=JUST_NODE):
...
def bake_labels(zone_results, params, status_cb):
    ...
    for cat in CATEGORIES:
        ...
        root = zd["root"]
        add_label(root, cat["key"], params.heading_pt, head_c, layer, cat["key"], params)
        zoom = cat.get("zoom", "")
        if zoom:
        ...
        for mid, short, tip in zd.get("tips", []):
            add_label(tip, short, params.subtopic_pt, tip_c, layer, mid + "_SUB", params)
            id_dy = text_height_mm(params.subtopic_pt, params) * 0.85
            add_label(
                rg.Point3d(tip.X, tip.Y + id_dy, 0),
                mid, params.branch_pt, id_c, layer, mid, params,
            )
    ...
    for mid, rid, verb in zone_results.get("_rels", []):
        add_label(
            mid,
            "{}  {}".format(rid, verb),
            params.branch_pt, rel_col, layer, rid, params,
        )

    unit = "print points" if getattr(params, "size_unit", "mm") == "pt" else "millimetres"
    status_cb(
        "Text height in the model (mm): title {:.2f}, heading {:.2f}, sub {:.2f}, "
        "branch {:.2f}, legend {:.2f}. Size unit = {}. "
        "Print-pt mode uses 1 pt = 0.353 mm, so GUI 2.0 becomes 0.71 mm — "
        "that is why older generates looked like 0.5–1.".format(
...
def generate_diagram(params, status_cb):
    ...
    rs.ZoomExtents()
    status_cb(
        "V12: legend text black (colour pickers per type). Size unit millimetres by default.\n"
...
class StigmergyForm(eforms.Form):
    ...
    def __init__(self):
        ...
        self.Title = "A1 Stigmergy  V12  v1.5  (text colours + mm sizes)"
        self.Resizable = True
    ...
    def _build_ui(self):
        ...
        settings.AddRow(w.section("Text -- Futura, colours + size"))
        ...
        self._size_unit = w.combo(
            ["millimetres (Rhino height)", "print points (x 0.353 mm)"],
            0,
            width=240,
        )
        settings.AddRow(w.row("Size unit", self._size_unit))
        settings.AddRow(w.lbl(
            "GUI range is 0.1–80, not 0.5–1. Millimetres: 2.0 = 2.0 mm. Print pt: 2.0 = 0.71 mm.",
            size=7.5,
        ))
        ...
        settings.AddRow(w.row2("Title", self._title_pt, self._title_col))
        ...
        settings.AddRow(w.row2("Relation / loop", w.lbl(""), self._rel_col))
        self._text_scale = w.num(self.params.text_scale, 0.1, 20, dec=1, inc=0.1)
        settings.AddRow(w.row("Text scale (multiply all)", self._text_scale))
        ...
        self.Content = outer
        self.status_cb(
            "V12: legend text black; colour chip next to each type. Size unit = millimetres.\n"
            "Print-pt mode still available — 2.0 pt used to bake as 0.71 mm."
        )
    ...
    def _read_params(self):
        ...
        p.size_unit = "pt" if int(self._size_unit.SelectedIndex or 0) == 1 else "mm"
