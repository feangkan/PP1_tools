"""
Assessment 1 stigmergy generator  V8 / v1.1
...
LAYOUT  v1.1 -- mycelial / nerve mesh (fast + animated)
------------------------------------------------------------
Gold REGISTRATION trunk is Victoria's linear gated pathway (Trust/Risk,
START, Stages 1-5, fail/re-queue, ejection). Other colour groups use the
same ride grammar: START / Stage / Gate / Loop / ejection-analog.
...
CATEGORIES = [
    {
        "key": "ETHICS",
        "zoom": "Duty past the fee-payer — a loop, not a slogan",
        "colour": (176, 106, 122),
        "hub": (-165, 38),
        "grow_dir": (-0.85, 0.35),
        "zone_radius": 36,
        ...
        "subs": [
            ("H1", "START: knowledge becomes power"),
            ("H2", "Stage: ethics answers that power"),
            ("H3", "Client not in the room"),
            ("H4", "Gate: profession is not a business"),
            ("H5", "Fees do not shrink judgment"),
            ("H6", "No good or bad architects — only degree"),
            ("H7", "Searchlight or lantern"),
            ("H8", "Loop: collective test — back into knowledge"),
        ],
    },
    {
        "key": "LEGISLATION",
        "zoom": "Instruments that can be rewritten — write, interpret, amend",
        "colour": (91, 124, 153),
        "hub": (-78, 48),
        "grow_dir": (-0.25, 0.95),
        "zone_radius": 40,
        ...
        "subs": [
            ("L1", "START: Act gives legal form"),
            ("L2", "Gate: who may use the title"),
            ("L3", "Cannot sign undone work"),
            ("L4", "Resign rather than breach — withdraw, not fudge"),
            ("L5", "Fire-isolated stair — public over payer"),
            ("L6", "Gate: NCC class and type"),
            ("L7", "Fail analog: professional vs prohibited courts"),
            ("L8", "Loop: written → interpreted → amended"),
    ...
    {
        "key": "RESEARCH",
        "zoom": "Codes change what may be built — an amendment path",
        "colour": (77, 143, 106),
        "hub": (-52, -52),
        "grow_dir": (-0.35, -0.90),
        "zone_radius": 32,
        ...
        "subs": [
            ("N1", "START: fit for purpose, then performance"),
            ("N2", "Codes are co-authored — not a closed file"),
            ("N3", "Stage: bathrooms into the NCC"),
            ("N4", "Stage: carbon written as a limit"),
            ("N5", "Gate: optional becomes ordinary"),
            ("N6", "Loop: one good makes another problem"),
        ],
    },
    {
        "key": "PROCUREMENT",
        "zoom": "Contract can strip design control — a degradation path",
        "colour": (52, 80, 112),
        "hub": (-158, -42),
        "grow_dir": (-0.92, -0.25),
        "zone_radius": 28,
        ...
        "subs": [
            ("P1", "START: traditional AS4000 — keep design"),
            ("P2", "Stage: design and construct AS4902"),
            ("P3", "Stage: novated D and C"),
            ("P4", "Evidence: Institute survey 2019"),
            ("P5", "Not ejected: title intact, authorship gone"),
        ],
    },
    {
        "key": "REGISTRATION",
    ...
    {
        "key": "EMPLOYMENT",
        "zoom": "One project, five stages — a loop with legal gates",
        "colour": (166, 138, 91),
        "hub": (-92, -68),
        "grow_dir": (-0.55, -0.80),
        "zone_radius": 32,
        ...
        "subs": [
            ("W1", "START: client + stakeholders write the brief"),
            ("W2", "Stage: Feasibility → Sketch → Developed"),
            ("W3", "Stage: Documentation → Contract administration"),
            ("W4", "Gate: Town Planning (Council)"),
            ("W5", "Gate: Building Surveyor"),
            ("W6", "Loop: findings return to the brief"),
    ...
    {
        "key": "EDUCATION",
        "zoom": "Judgment trained, then kept alive — not a one-off stamp",
        "colour": (107, 155, 107),
        "hub": (-8, 52),
        "grow_dir": (0.05, 0.98),
        "zone_radius": 30,
        ...
        "subs": [
            ("E1", "START: university under AQF"),
            ("E2", "Stage: Part 1 then Part 2 — gated into APE"),
            ("E3", "Judgment over competency checklist"),
            ("E4", "Integrity: head · heart · hand"),
            ("E5", "Loop: CPD formative — after the title, not only after a breach"),
            ("E6", "Classic or neo-profession — mark your position"),
        ],
    },
    {
        "key": "BODIES",
        "zoom": "Institute advances; Board accounts — two gates, not one",
        "colour": (138, 107, 168),
        "hub": (125, 38),
        "grow_dir": (0.90, 0.35),
        "zone_radius": 34,
        ...
        "subs": [
            ("B1", "Gate A: AIA is voluntary — cannot eject from the title"),
            ("B2", "Gate B: ARBV is statutory — can eject"),
            ("B3", "AACA sets the national test"),
            ("B4", "Network filling AIA gaps"),
            ("B5", "Fragmentation is the problem"),
            ("B6", "Awards write 'good' — not a legal gate"),
    ...
    {
        "key": "POWER",
        "zoom": "Not a linear ride — overlap across the other gates",
        "colour": (176, 80, 64),
        "hub": (62, 8),
        "grow_dir": (0.65, 0.15),
        "zone_radius": 24,
        ...
        "subs": [
            ("K1", "Not a pathway: interlocking seats"),
            ...
            ("K6", "Change: who writes rules / keeps authorship"),
        ],
    },
]
...
INTRA_LOOPS = [
    ...
    ("ETHICS", "H8", "H1", "loop back"),
    ("EDUCATION", "E5", "E3", "CPD formative loop"),
    ("RESEARCH", "N6", "N1", "one good, another problem"),
    ("LEGISLATION", "L8", "L1", "write–interpret–amend"),
]
...
def generate_diagram(params, status_cb):
    ...
    try:
        ...
        status_cb("Mesh: {} local, {} nerve bridges, {} capillaries, {} junctions.".format(
            n_mesh, n_bridge, n_cap, n_junc))
    ...
    rs.ZoomExtents()
    status_cb(
        "V8 pathway content + circle legend. Hide pipes before vector export.\n"
...
class StigmergyForm(eforms.Form):
    ...
    def __init__(self):
        ...
        self.Title = "A1 Stigmergy  V8  v1.1  (pathway content + circle legend)"
        self.Resizable = True
    ...
    def _build_ui(self):
        ...
        self._draw_loops = w.check("Dotted intra-group loops (fail / re-attempt)", True)
        ...
        settings.AddRow(w.section("Line weight -- real pipes, visible zoomed out"))
        self._highway = w.num(self.params.highway_line, 0.3, 4.0, dec=2, inc=0.05)
        settings.AddRow(w.row("Highway trunks", self._highway))
        ...
        self.Content = outer
        self.status_cb(
            "V8: gold path = Victoria flowchart. Other groups = START/Stage/Gate/Loop.\n"
            "Legend = colour circles. Futura pt sizes as in the font panel."
        )
