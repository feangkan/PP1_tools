"""
Assessment 1 stigmergy generator  V16 / v1.9
...
Futura catalog text: pick a colour per type; legend defaults to black.
Text sizes are millimetres (what Rhino Properties shows). 2.0 = 2.0 mm.
After Generate, use Update text to restyle size and colour without redrawing.
...
class GrowthParams(object):
    def __init__(self):
        ...
        self.title_pt = 2.0
        self.heading_pt = 2.0
        self.subtopic_pt = 2.0
        self.branch_pt = 0.1
        self.legend_pt = 1.0
        self.text_scale = 1.0
        self.use_group_colour = True
...
def add_label(pt, text, pt_size, colour, layer, name, params, justify=JUST_NODE, kind=None):
    """Place Futura on a point. Default justification is middle-center (on the node)."""
...
def bake_loops(zone_results, params, status_cb):
    ...
    for group, a_id, b_id, label in INTRA_LOOPS:
        ...
        add_label(
            ...
            label, params.branch_pt, getattr(params, "relation_colour", (0, 0, 0)),
            layer, "{}-{}".format(a_id, b_id), params, kind="loop",
        )
        n += 1
        anim_tick()
    if n:
...
def bake_labels(zone_results, params, status_cb):
    ...
    for mid, rid, verb in zone_results.get("_rels", []):
        add_label(
            mid,
            "{}  {}".format(rid, verb),
            params.branch_pt, rel_col, layer, rid, params, kind="relation",
        )

    status_cb(
        "Text height in the model (mm): title {:.2f}, heading {:.2f}, sub {:.2f}, "
        "branch {:.2f}, legend {:.2f}. Numbers in the GUI are millimetres.".format(
...
def style_for_kind(kind, oid, params):
    ...
    sizes = {
        ...
        "relation": params.branch_pt,
        "loop": params.branch_pt,
...
def apply_text_style(tid, height, colour, font):
    try:
        rs.ObjectColorSource(tid, 1)
    except Exception:
        pass
...
def update_baked_text(params, status_cb):
    ...
    for oid in ids:
        ...
        apply_text_style(oid, text_height_mm(size, params), colour, font)
    ...
    status_cb("Updated {} ({}). Geometry unchanged.".format(scope, bits or "0 objects"))
    return len(ids)
...
def generate_diagram(params, status_cb):
    ...
    try:
        ...
        status_cb("Done in {:.1f}s (clean highways{}).".format(
            elapsed,
    ...
    rs.ZoomExtents()
    status_cb(
        "V16: clean highways (3 smooth curves). Mesh and side filaments off.\n"
        "Update text still restyles size and colour without regenerating."
    )
...
class StigmergyForm(eforms.Form):
    ...
    def __init__(self):
        ...
        self.Title = "A1 Stigmergy  V16  v1.9  (clean highways)"
        self.Resizable = True
    ...
    def _build_ui(self):
        ...
        settings.AddRow(w.section("Text -- Futura, sizes in mm"))
        ...
        self._use_group_col = w.check("Hub / tip text uses group colour", True)
        ...
        settings.AddRow(w.row2("Relation / loop colour", w.lbl(""), self._rel_col))
        ...
        self.Content = outer
        self.status_cb(
            "V16: clean highways like pic 1. Mesh / side filaments off. Update text for size/colour."
        )
