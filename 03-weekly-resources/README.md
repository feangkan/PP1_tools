# Weekly resources

One folder per teaching week. Weeks 1–7 exist; add `week-08`, … as the semester continues.

## Layout per week

```
week-XX/
  README.md           # topic, goals, file index (copy from templates/week-readme.md)
  lecture-notes.md    # optional
  tutorial-notes.md   # optional
  readings/           # optional: PDFs, articles
  links.md            # optional: LMS / video links
```

## Index

| Week | Topic | Status |
|------|-------|--------|
| [01](week-01/) | What is a profession? (Beaton — professionalism, altruism, asymmetrical knowledge) | Full pack — `05-diagrams/week-01/`; summary; bibliography |
| [02](week-02/) | Models for understanding a profession (Traits, Continuum, Apparatus, Kuhn, Divided Line) + Sciulli (2008) | Full pack — `05-diagrams/week-02/` |
| [03](week-03/) | Origin of the Architects Act; APE & registration; compliance culture; Molander (discretion & accountability) | Full pack — `05-diagrams/week-03/` |
| [04](week-04/) | The Good and the Bad Architect (pt 1); recognition machinery — awards, journals, media; the "collective test" | Full pack — `05-diagrams/week-04/` |
| 05 | No standalone pack — building-code pre-reading (Lui, Hill) pulled directly into Assessment 1 | see `05-diagrams/assessment-1/` |
| [06](week-06/) | The Entrepreneur + The Collective — innovation/value-capture (Helen) + AI as leverage (Gwyl Jahn); also where Assessment 2 development began | Full pack — `05-diagrams/week-06/`; summary; feeds Assessment 2 (`05-diagrams/assessment-2/`) |
| [07](week-07/) | The Architect as Worker (Helen) + Union for Architectural Workers (Tom Marshall, UAW) — structural vs individual responses, wellbeing research, FAF/RIBA case, union economics | Resources uploaded; summary/diagram pack not yet generated |

## Agent prompts

- Summarise a week → save under `04-summaries/`
- Diagram a week’s process → save under `05-diagrams/`
- Answer a question from one week only → save under `07-q-and-a/`
